...
class CampaignController extends Controller
{
    ...
    public function storeItem(Request $request)
    {
        ...
        $campaign->discount = $request->discount_type == 'amount' ? $request->discount : $request->discount ?? 0;
        ...
        $campaign->store_id = $request->store_id ?? 1;
        ...
        $attributeValues = $request->input('attributeValue', []); // All submitted attribute values
        $checkedAttributes = $request->input('attribute', []);   // Only the checked attributes
        $result = [];
        foreach ($attributeValues as $attributeValue) {
            $isActive = in_array($attributeValue, $checkedAttributes) ? 1 : 0;
...
//        $campaign->allergies()->sync($allergy_ids);
    ...
    public function updateItem(ItemCampaign $campaign, Request $request)
    {
        ...
        $campaign->stock= $request->current_stock;
        $campaign->save();
        $attributeValues = $request->input('attributeValue', []); // All attribute names
        $checkedAttributes = $request->input('attribute', []);   // Only the checked ones
        foreach ($attributeValues as $attributeValue) {
            $isActive = in_array($attributeValue, $checkedAttributes) ? 1 : 0;
            ItemAttributes::updateOrCreate(
                ['name' => $attributeValue, 'item_id' => $campaign->id], // Match by attribute name and entity
                ['isActive' => $isActive]                                // Set active status
            );
        }
