...
class CartController extends Controller
{
    public function get_carts(Request $request)
    {
        ...
        ->map(function ($data) {
            ...
            // $data->components = json_decode($data->attributes,true)??[];
...
//            }
        ...
        $cart->module_id = $request->header('moduleId') ?? $request->moduleId ?? 1;
        ...
        $cart->item_type = $request->model??'Item';
        $cart->price = $request->price??1;
        $cart->quantity = $request->quantity;
        $cart->variation = isset($request->variation)?json_encode($request->variation):json_encode([]);
        $cart->components = $request->components ;
    // $cart->attributes = json_decode($cart->attributes,true);
        ...
        ->map(function ($data) use($request) {
			...
			$data->item = Helpers::cart_product_data_formatting($data->item, $data->variation,$data->add_on_ids,
            $data->add_on_qtys, false, app()->getLocale());
        // 	$carts['components'] = \App\Models\Item::find($request->item_id)->components;
...
// 		dd($carts['components']);
    ...
    public function update_cart(Request $request)
    {
        ...
        $cart->components = $request->components ??$cart->components;
        $cart->save();
