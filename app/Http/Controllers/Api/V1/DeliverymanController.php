...
class DeliverymanController extends Controller
{
    ...
    {
        $dm = DeliveryMan::with(['rating', 'zones'])->where(['auth_token' => $request['token']])->first();
        if ($dm) {
            $dm['zone_ids'] = $dm->zones->pluck('id')->toArray();
            if (!in_array($dm->zone_id, $dm['zone_ids'])) {
                $dm['zone_ids'][] = $dm->zone_id;
            }
            unset($dm['zones']);
        }
    ...
    {
        $dm = DeliveryMan::with(['rating'])->where('active', 1)->count();
        return response()->json($dm, 200);
    } //listActive
    ...
    public function get_latest_orders(Request $request)
    {
        ...
        if ($dm->type == 'zone_wise') {
            $zoneIds = $dm->zones->pluck('id')->toArray();
            if (!in_array($dm->zone_id, $zoneIds)) {
                $zoneIds[] = $dm->zone_id;
            }
    ...
    public function update_order_status(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'order_id' => 'required',
            'status' => 'required|in:confirmed,canceled,picked_up,delivered,handover,delivered_and_take_money,delivered_and_not_take_money',
    ...
    public function get_notifications(Request $request)
    {
        ...
        $notifications = Notification::active()->where(function ($q) use ($zoneIds) {
            $q->whereNull('zone_id')->orWhereIn('zone_id', $zoneIds);
