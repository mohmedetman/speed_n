...
//            'contact_person_number' => $request->contact_person_number ? ($request->user ? $request->contact_person_number :str_replace('+', '', $request->contact_person_number)) : ($request->user?$request->user->phone:''),
       ...
       // $order->order  $order->order_amount = $request['order_amount'];
          // $order->order_amount = ceil($request['order_amount']);
             $order->order_amount = 14;
        ...
        $order->delivery_charge = ceil($delivery_charge) ?? 0;
        ...
        $order->components = json_decode($request->cart);    
        ...
        if ($request->order_type !== 'parcel') {
            ...
            $order->order_amount = ceil($total_price + $tax_a + $order->delivery_charge);
            ...
            $order->delivery_charge = ceil($original_delivery_charge) ?? 0;
            $order->original_delivery_charge = ceil($original_delivery_charge, config('round_up_to_digit'));
            $order->order_amount = ceil($order->delivery_charge, config('round_up_to_digit'));
        }
        ...
        //DM TIPS
        $order->order_amount = ceil($order->order_amount + $order->dm_tips + $order->additional_charge + $order->extra_packaging_amount);
...
//dd( $order->order_amount) ;
 if($request->store_id && !isset($free_delivery_over) && $request->order_type == 'delivery' ){
             $delivery_store_price = Store::where('id',$request->store_id)->first()->delivery_price ?? 33;
             if($order->delivery_charge < $delivery_store_price ){

                
                $order->delivery_charge =  $delivery_store_price ;
               $order->order_amount = $request->order_amount  ;
        
if ($order->original_delivery_charge < $delivery_store_price) {
            $order->original_delivery_charge = $delivery_store_price;
        }


       //$order->order_amount += ($delivery_store_price - $order->delivery_charge);
             }
            }
...
->first()->value ?? 5 ;
            CheckOrderStatus::dispatch($order->id)->delay(now()->addMinutes($time));
            if ($request->order_type !== 'parcel') {
            ...
            return response()->json([
                ...
                // 'components' => \App\Models\Item::find($request->item_id)->components,
    ...
    public function prescription_place_order(Request $request)
    {
        ...
        $order->store_id = $request['store_id'];
        $order->delivery_charge = ceil($order->delivery_charge($delivery_charge))  ?? 0;
        $order->original_delivery_charge = round($original_delivery_charge, config('round_up_to_digit'));
        $order->delivery_address = json_encode($address);
        $order->schedule_at = $schedule_at;
        $order->scheduled = $request->schedule_at ? 1 : 0;
