...
class OrderController extends Controller
{
    ...
    public function place_order_old(Request $request)
    {
        $validator = Validator::make($request->all(), [
...
//            'contact_person_number' => $request->contact_person_number ? ($request->user ? $request->contact_person_number :str_replace('+', '', $request->contact_person_number)) : ($request->user?$request->user->phone:''),
        ...
        $order->store_id = 0;
        $order->store_id_old = $request['store_id'];
        ...
        foreach ($deviceTokens as $token) {
            $data = [
                "message" => [
                    ...
                    "notification" => [
                        "title" => 'new order',
                         "body" => (string)$order->id, // Convert order ID to string
                    ],
                ],
            ];
            try {
...
//        })
    ...
    public static  function send_not ($order)
      {
          ...
          foreach ($deviceTokens as $token) {
              $data = [
                  "message" => [
                      ...
                      "notification" => [
                          "title" => 'new order',
                          "body" => ['store_id'=>$order->store_id_old,'customer_id'=>$order->user_id],
                      ],
                  ],
              ];
              try {
