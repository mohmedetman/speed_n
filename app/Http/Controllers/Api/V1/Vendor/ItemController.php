...
//                dd(Item::with('components')->orderBy('created_at','desc')->first());
  ...
  if ($request->has('components')) {
    ...
    {
        try {
            $item = Item::withoutGlobalScope('translate')->with(['tags','components'])->where('id',$id)
            ->first();
...
\App\Models\ItemAttributes::where('item_id', $p->id)->delete();
