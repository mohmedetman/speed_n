...
//   dd(4);
    ...
    foreach ($components as $component) {
        $component = trim($component);
                if (!empty($component)) {
            \App\Models\ItemAttributes::create([
                'item_id' => $food->id,
                'name' => $component,
            ]);
        }
...
// }
    ...
    public function edit(Request $request,$id)
    {
        ...
        $conditions = CommonCondition::get(['id','name']);
        $brands = Brand::all();
               $components = \App\Models\ItemAttributes::where('item_id',$id)->pluck('name')->toArray();
        $components  =  implode(',',$components) ;
              
        return view('vendor-views.product.edit', compact('product', 'product_category', 'categories','module_data', 'temp_product','conditions','brands','components'));
    }
    ...
    if (!empty($components)) {
        $existingAttributes = \App\Models\ItemAttributes::where('item_id', $id)->pluck('name')->toArray();
        
        $componentsToDelete = array_diff($existingAttributes, $components);
          \App\Models\ItemAttributes::where('item_id', $id)
                                  ->whereIn('name', $componentsToDelete)
                                  ->delete();
        
        foreach ($components as $component) {
            if (!empty($component)) {
                $itemAttribute = \App\Models\ItemAttributes::where('item_id', $id)
                                                         ->where('name', $component)
                                                         ->first();
                
                if ($itemAttribute) {
                    $itemAttribute->update([
                        'name' => $component, // You can update other fields as needed
                    ]);
                } else {
                    \App\Models\ItemAttributes::create([
                        'item_id' => $id,
    ...
    public function list(Request $request)
    {
        ...
        $sub_tab = request()->get('sub_tab') ?? '';
       
        $store =\App\CentralLogics\Helpers::get_loggedin_user();
 ...
 ->when($request->sub_tab,function($query) use($request){
            if($request->sub_tab ==="inactive-items") {
                return $query->where('status',0);
            }
            if($request->sub_tab ==="active-items") {
                return $query->where('status',1);
            } //temp_product
            if($request->sub_tab ==="rejected-items") {
                return $query->whereHas("temp_product",function($q)use($request){
                    return $q->where("is_rejected",1);
                });//pending-items
            } //temp_product
            if($request->sub_tab ==="pending-items") {
                return $query->whereHas("temp_product",function($q)use($request){
                    return $q->where("is_rejected",0);
                });//pending-items
            } //temp_product
        ...
        return view('vendor-views.product.list', compact('items', 'category', 'type','sub_categories','sub_tab','store'));
    }
